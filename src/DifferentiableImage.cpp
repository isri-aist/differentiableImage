...
void DifferentiableImage::init(const std::string &path_des, const std::string &path_cur, bool enable_display, const std::string &mask){
    ...
    try{
        ...
        if(enableDisplay){
            ...
            disp_upsampled_subsampled_I_to_upsub.init(upsampled_subsampled_I_to_upsub, 100 + 3*I_des.getWidth() + 5, 100, "upSubGauss");
...
double DifferentiableImage::compute(){
    for(unsigned int kernel = 0 ; kernel < nbKernels ; kernel++, sigma+=sigmaStep){
        ...
        prRegularlySampledCPImage<unsigned char> IP_cur(
            I_cur.getHeight(),
            I_cur.getWidth()); // the regularly sample planar image to be set from the
                        // acquired/loaded perspective image
        prRegularlySampledCPImage<float> GP_cur(
            I_cur.getHeight(),
		...
		ceres::Grid2D<unsigned char, 1> arrayDes(subsampled_I_to_upsub.bitmap, 0, subsampled_I_to_upsub.getHeight(), 0, subsampled_I_to_upsub.getWidth());
        ...
        if (kernel == nbKernels - 1){
            ...
            for (size_t i = 0; i < v_cost.size() - 1; ++i) {
                if ((v_cost[i] <= val_95 && v_cost[i + 1] >= val_95) || (v_cost[i] >= val_95 && v_cost[i + 1] <= val_95)) {
                    ...
                    interpolated_sigma = v_sigma[i] + t * (v_sigma[i + 1] - v_sigma[i]); //interpolate to find lambda_g at 95% of the cost
                    break;
                }
            }
